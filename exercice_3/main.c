...
int main(int argc, char** argv){
    ...
    do {
        ...
        if (menuChoice == 3) {
            ...
            if (number1 == sum) printf("\nLa propriete est valide pour le nombre %d", nbsortie);
            else printf("\nCette propriete n'est pas valide pour le nombre %d", nbsortie);
        }
        ...
        if (menuChoice == 5) {
            ...
            for (j = borne1; j < borne2 ; ++j) {
                ...
                for(num1 = 0; num1 < lenght; num1++){ // permet de créer le nombre de boucle grâce à la variable num1
                    ...
                    nbsortie += (j % (max * 10) / max) * pow(10,num1);//permet d'ajouter le plus grand chiffre à gauche du nombre par ordre croissant
                ...
                if (j == sum) printf("Le nombre %d valide cette propriété !", i);
            }
        }
